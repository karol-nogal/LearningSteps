...
const Student1 = {
  ...
  DisplayFullInfo: function () {
    console.log(
      this.name + " " + this.adress + " " + this.IDnumber + " " + this.Profile
    );
  },
  ChangeID: function (a) {
    this.IDnumber = a;
...
Render.prototype.renderRow = function (name, surname, age, role, index) {
  return (
    ...
    "</div><div>" +
    role +
    '</div><button onclick="dbInstance.removePerson(' +
    index +
    ')">Jednak lubię tą osobę</button></div>'
  );
};
...
Render.prototype.setHtml = function (database) {
  ...
  database.map(function (person, index) {
    self.container.innerHTML += self.renderRow(
      person.name,
      person.surname,
      person.age,
      person.role,
...
class People {
  ...
  displayFullInfo() {
    console.log(
      `${this.profile} ${this.name} lat ${this.age} zamieszkaly w ${this.adress} o nr ${this.IDnumber}`
    );
  }
  changeID(a) {
    this.IDnumber = a;
